...
class auth_plugin_authssp extends DokuWiki_Auth_Plugin
{
    ...
    public function trustExternal()
    {
        ...
        $this->debug(sprintf("Checking for simpleSAMLphp session..."));
    ...
    protected function retrieveUserGroups()
    {
        ...
        if (is_array($groupSourceConfig)) {
            foreach ($groupSourceConfig as $sourceName) {
        	$groups = array_merge($groups, $this->getShibVar($sourceName, true));
            }
        } else {
